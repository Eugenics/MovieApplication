...
import com.eugenics.movieapplication.data.datasources.pagingsource.SearchPagingDataSource
...
class RemoteDataSourceImpl(private val api: TmdbApi) : RemoteDataSource {
    ...
    override fun searchMovie(queryString: String): Flow<PagingData<Movie>> =
        Pager(
            config = PagingConfig(pageSize = PAGE_SIZE, enablePlaceholders = false),
            pagingSourceFactory = {
                SearchPagingDataSource(api = api, queryString = queryString)
            }
        ).flow
}